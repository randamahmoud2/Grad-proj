...
const Table2Down = (props) => {
  ...
  const table2DownJson = dentalChartConfig.tables.find(t => t.name === "Table2Down");
  ...
  useEffect(() => {
    if (isChartReady && lowerTeeth.length > 0 && table2DownJson && teethController) {
      ...
      table2DownJson.teeth.forEach(tooth => {
      ...
      table2DownJson.teeth.forEach(toothData => {
  ...
  }, [isChartReady, lowerTeeth, table2DownJson, teethController]);
  ...
  const handleImplantChange = (e, toothId) => {
    ...
    if (patientChartManager && table2DownJson) {
      const updatedTableData = {
        ...table2DownJson,
        teeth: table2DownJson.teeth.map(t => 
          t.id === toothId ? { ...t, implant: isChecked } : t
        )
      };
      patientChartManager.updateTableData("Table2Down", updatedTableData);
    }
  };
    ...
    <table className="table" style={{border: "none"}}>
      <tbody>
        ...
        <tr>
          ...
          {lowerTeeth.map(x => (
            <td key={`mobility-${x.Id}`} style={{whiteSpace: 'nowrap'}}>
              <input
        ...
        <tr>
          ...
          {lowerTeeth.map(x => (
            <td key={`implant-${x.Id}`} style={{whiteSpace: 'nowrap'}}>
              <input
